...
post() {
  [[ -d "$HOME/.local/share/garrytan/gstack" ]] || return 0
  mkdir -p "$HOME/.claude/skills"
  ln -sfn "$HOME/.local/share/garrytan/gstack" "$HOME/.claude/skills/gstack"
  local _d
  for _d in "$HOME/.local/share/garrytan/gstack"/*/; do
