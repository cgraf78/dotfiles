...
set_prompt() {
  ...
  # Only set once — re-calls from overlay/local prompt overrides would clobber
  # hooks appended by 70-integrations.bash (direnv, zoxide, etc.).
...
# Args: $1 - alias to display (e.g., "short-host").
...
set_hostname_alias() {
  ...
  set_prompt "$1"
  if [ -n "$TMUX" ]; then
    tmux set -g @hostname_alias "$1" &>/dev/null &
    disown 2>/dev/null
  fi
  return 0
}
