...
# reported by Windows itself.
if [[ -z "${WINHOME:-}" ]]; then
  if dot_wsl_windows_home; then
    export WINHOME="$REPLY"
  fi
fi
...
wcode() {
  ...
  target="$(wslpath -w "${1:-.}")"
  code_exe="${WINHOME:-}/AppData/Local/Programs/Microsoft VS Code/Code.exe"

  if [[ -n "${WINHOME:-}" && -x "$code_exe" ]]; then
