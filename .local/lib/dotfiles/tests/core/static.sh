...
MOCK
  ...
  while IFS= read -r _ci_line; do
    ...
    if ((_ci_in_shell_with)) &&
      [[ "$_ci_code" =~ ^[[:space:]]{8}HOME=\"\$PWD\"[[:space:]]+PATH=\"\$PWD/\.local/bin:\$PATH\"[[:space:]]+bash[[:space:]]+\.local/lib/dotfiles/tests/android-ci-smoke[[:space:]]*$ ]]; then
  ...
  while IFS= read -r _tracked; do
    [[ "$_tracked" != .local/lib/dot/tests/* ]] || continue
...
for name in sorted(config_names):
    ...
    # SSHD is a privileged pre-sync policy with its own test and deliberately
    # has no ordinary user merge-hook script.
    if name == "sshd":
        continue
