...
# Resolve the standalone engine used by client-policy tests. Source-worktree
# runs deliberately prefer an explicit override, then the host development
# checkout, then the Shdeps-managed checkout. The returned path is physical so
# extension trust and provenance assertions see one stable identity.
_test_dot_root() {
