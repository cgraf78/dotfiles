...
- `bootstrap-shortcut-test` pipes the local standalone installer through
  `--init` against a local client origin and drift-checks the documented public
  URL/client binding. It never reaches the developer's real GitHub account.
- `engine-equivalence-test` archives the immutable pre-extraction embedded
  engine and the reviewed standalone candidate into independent HOME/XDG
  environments, runs the same ignore and cron policy twice, and compares the
  complete normalized filesystem inventories plus ordered external-effect
  transcripts. Its fixture permits only the explicit engine-ownership path
  change from `.local/lib/dot` to `.local/lib/dotfiles`.
