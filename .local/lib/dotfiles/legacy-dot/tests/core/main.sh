...
dot_core_test_main() {
  ...
  _assert_file_content "fixture engine root: copies explicit core modules" \
    ...
    "$fixture_target_home/.local/lib/dotfiles/legacy-dot/core/sentinel.sh"
  _assert_file_content "fixture engine root: copies explicit doctor modules" \
    '# explicit doctor sentinel' \
    "$fixture_target_home/.local/lib/dotfiles/legacy-dot/core/doctor/sentinel.sh"
  _assert_file_content "fixture engine root: copies explicit repository modules" \
    '# explicit repo sentinel' \
    "$fixture_target_home/.local/lib/dotfiles/legacy-dot/core/repos/sentinel.sh"

  mkdir -p "$fixture_target_home/.local/lib/dotfiles/legacy-dot/core"
  fixture_saved_ensure_shdeps=$(declare -f _ensure_shdeps)
  cat >"$fixture_target_home/.local/lib/dotfiles/legacy-dot/core/init.sh" <<'INIT'
...
MOCK
  ...
  _done_hint_script=$(
    printf '%s' \
      '. "$HOME/.local/lib/dotfiles/legacy-dot/core/constants.sh"; ' \
      '. "$HOME/.local/lib/dotfiles/legacy-dot/core/log.sh"; ' \
      '. "$HOME/.local/lib/dotfiles/legacy-dot/core/progress-ui.sh"; ' \
