...
MOCK
  ...
  while IFS= read -r _ci_line; do
    ...
    if ((_ci_in_shell_with)) &&
      [[ "$_ci_code" =~ ^[[:space:]]{8}HOME=\"\$PWD\"[[:space:]]+PATH=\"\$PWD/\.local/bin:\$PATH\"[[:space:]]+bash[[:space:]]+\.local/lib/dotfiles/legacy-dot/tests/android-ci-smoke[[:space:]]*$ ]]; then
...
legacy_script_root = root / ".local/lib/dotfiles/legacy-dot/core/merge-hooks.d"
script_root = root / ".local/lib/dotfiles/legacy-dot/core/merge-hooks"

if legacy_script_root.exists():
    print(f"{legacy_script_root.relative_to(root)}: merge hook implementations belong under .local/lib/dotfiles/legacy-dot/core/merge-hooks")
...
for rel in sorted(config_scripts):
    print(f"{rel}: merge hook scripts belong under .local/lib/dotfiles/legacy-dot/core/merge-hooks")
...
PY
  ...
  _core_boundary_scanner="$_lint_root/.local/lib/dotfiles/legacy-dot/tests/core-boundary-inventory.py"
  ...
  for _core_boundary_spec in \
    ...
    awk 'NF && $0 !~ /^[[:space:]]*#/' \
      "$_lint_root/.local/lib/dotfiles/legacy-dot/tests/fixtures/$_core_boundary_fixture" \
  ...
  _core_boundary_fixture_core="$_core_boundary_fixture_root/.local/lib/dotfiles/legacy-dot/core"
...
#!/usr/bin/env -S bash
core="$HOME/.local/lib/dotfiles/legacy-dot/core"
. "$core/xdg.sh"
FIXTURE
  cat >"$_core_boundary_fixture_root/.local/bin/scope" <<'FIXTURE'
#!/usr/bin/env bash
probe() { local core="$HOME/.local/lib/dotfiles/legacy-dot/core"; :; }
run() { local core=/tmp; . "$core/xdg.sh"; }
FIXTURE
  # shellcheck disable=SC2016  # generated fixture must retain literal $HOME.
  _core_boundary_heredoc_literal='$HOME/.local/lib/dotfiles/legacy-dot/'core/xdg.sh
...
FIXTURE
  ...
  ln -s '../lib/dotfiles/legacy-dot/'core/xdg.sh \
    "$_core_boundary_fixture_root/.local/bin/direct-link"
  _core_boundary_fixture_owner=.local/lib/dotfiles/legacy-dot/core
...
for rel in tracked.read_text().splitlines():
    ...
    if not rel or rel.startswith((
        ".local/lib/dotfiles/legacy-dot/tests/",
...
PY
  ...
  _shell_dir_probe=$(
    ...
    . "$_lint_root/.local/lib/dotfiles/legacy-dot/core/constants.sh"
