...
DOTRUNNER
  ...
  _assert_contains "dot-test shards: static wrapper suppresses runner coverage" \
    ...
    "$(cat "$HOME/.local/lib/dotfiles/legacy-dot/tests/core-static-test")"
...
DOTRUNNER
  ...
  mkdir -p "$_dot_runner_source_home/.local/bin" \
    ...
    "$_dot_runner_host_home/.local/lib/dotfiles/legacy-dot" \
  ...
  cat >"$_dot_runner_host_home/.local/lib/dotfiles/legacy-dot/ui.sh" <<'DOTRUNNER'
