...
# The rescue runtime is intentionally outside the standalone public-library
# namespace. The launcher exports its physical root; direct test/module loads
# derive the same root from this file so fixtures do not depend on HOME.
if [[ -z ${DOT_LEGACY_ROOT:-} ]]; then
  DOT_LEGACY_ROOT=${BASH_SOURCE[0]%/core/constants.sh}
fi
