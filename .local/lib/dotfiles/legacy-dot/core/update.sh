...
_dot_update_retired_generated_cache_file_ok() {
  local root=$1 path=$2 expected_uid=$3 expected_device=$4
  local base tag output path_uid path_device links

  case $path in
    "$root"/schemas/__pycache__/schema_policy.cpython-*.pyc) ;;
    *) return 1 ;;
  esac
  base=${path##*/}
  tag=${base#schema_policy.cpython-}
  tag=${tag%.pyc}
  case $tag in
    '' | *[!0-9]*) return 1 ;;
  esac
  [[ -f $path && ! -L $path ]] || return 1
  if output=$(stat -c '%u %d %h' "$path" 2>/dev/null); then
    :
  elif output=$(stat -f '%u %d %l' "$path" 2>/dev/null); then
    :
  else
    return 1
  fi
  read -r path_uid path_device links <<<"$output"
  [[ $path_uid == "$expected_uid" && $path_device == "$expected_device" &&
    $links == 1 ]]
}
  ...
  find "$root" -xdev -print0 2>/dev/null |
    while IFS= read -r -d '' path; do
      if [[ -d $path && ! -L $path ]]; then
        ...
        _dot_update_retired_generated_cache_file_ok \
          "$root" "$path" "$expected_uid" "$expected_device" || exit 1
      fi
    done
)

_dot_update_remove_retired_generated_cache() {
  local root=$1 cache identity expected_uid expected_device path

  cache=$root/schemas/__pycache__
  if [[ ! -e $cache && ! -L $cache ]]; then
    return 0
  fi
  [[ -d $cache && ! -L $cache ]] || return 1
  identity=$(_dot_update_path_owner_device "$root") || return 1
  expected_uid=${identity%% *}
  expected_device=${identity#* }
  for path in "$cache"/schema_policy.cpython-*.pyc; do
    [[ -e $path || -L $path ]] || continue
    _dot_update_retired_generated_cache_file_ok \
      "$root" "$path" "$expected_uid" "$expected_device" || return 1
    rm -f -- "$path" || return 1
  done
}
...
_dot_update_remove_retired_public_directory() {
  ...
  # root plus the old schema module's exact generated bytecode cache. Any
  # other link, file, or nonempty directory remains untouched and makes the
  ...
  _dot_update_remove_retired_generated_cache "$public_path" || return 1
  find "$public_path" -depth -type d -exec rmdir {} \; 2>/dev/null || return 1
  [[ ! -e $public_path && ! -L $public_path ]]
}

_dot_update_dot_client_surfaces_current() {
  local install_home=${SHDEPS_INSTALL_DIR:-$HOME/.local/share}
  local checkout public_path expected actual
  ...
  public_path=$HOME/.local/lib/dot
  expected=$checkout/lib/dot/public
  [[ -L $public_path ]] || return 1
  actual=$(readlink "$public_path") || return 1
  [[ $actual == "$expected" ]]
}
...
_dot_update_ensure_dot_client_surfaces() (
  ...
  # temporary preparation retry repairs that already-current fleet state.
  # shellcheck source=/dev/null
  . "$hook"
  declare -F post >/dev/null 2>&1 || return 1
  if [[ ${DOT_QUIET:-0} == 1 || ${SHDEPS_QUIET:-0} == 1 ]]; then
    post cgraf78/dot >/dev/null 2>&1
  else
    post cgraf78/dot
  fi
)
...
_dot_update_finalize() {
  ...
  if [[ "$shdeps_ready" -eq 1 ]] && declare -f shdeps_update &>/dev/null; then
    if _run_shdeps_update_ui; then
      if _dot_update_ensure_dot_client_surfaces; then
        _ui_stage_finish "${DOT_UI_SHDEPS_STATUS:-ok}" \
          "${DOT_UI_SHDEPS_SUMMARY:-dependencies checked}"
      else
        update_status=1
        _ui_stage_finish failed "Dot client surfaces unavailable"
      fi
    else
      update_status=1
