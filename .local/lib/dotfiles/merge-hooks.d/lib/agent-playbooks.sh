...
_dot_playbook_record_link_target() {
  local rel=$1 name=$2 path=$3 sync=${4:-git} rest prefix=''
  case $sync in
    git)
      rest=$rel
      while [[ $rest == */* ]]; do
...
_dot_playbook_link_matches() {
  local rel=$1 target=$3
  [[ -L $HOME/$rel && $(readlink "$HOME/$rel") == "$target" ]]
}

_dot_playbook_authority_files() {
  local manifest
  OVERLAY_AUTHORITY_MANIFESTS=()
  for manifest in "$DOT_OVERLAY_MANIFEST" "${DOT_OVERLAY_MANIFEST}.pending"; do
    [[ -e $manifest || -L $manifest ]] || continue
    _overlay_manifest_safe "$manifest" || return 1
    OVERLAY_AUTHORITY_MANIFESTS+=("$manifest")
  done
}
...
_dot_playbook_local_source_trusted() {
  ...
  for entry in "${OVERLAYS[@]+"${OVERLAYS[@]}"}"; do
    ...
    _dot_playbook_record_link_target "$rel" "$name" "$path" "$sync" || return 2
    expected="$REPLY"
...
_dot_playbook_overlay_files() {
  ...
  _dot_playbook_authority_files || return 1
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
      ...
      case "$REPLY_REL" in
        .config/agent-rules/playbooks.d/*.md)
          _dot_playbook_link_matches "$REPLY_REL" "$REPLY_OWNER" "$REPLY_TARGET" || continue
          ...
          if [[ "$local_status" -eq 0 ]]; then
            ...
            _dot_playbook_record_link_target "$REPLY_REL" "$REPLY_OWNER" \
