...
_gh_config_sources() {
  ...
  while IFS= read -r source; do
    _gh_sources+=("$source")
  done < <(_merge_hook_family_files_matching gh/config.d '*.yml' '*.yaml' '*.replace/*.yml' '*.replace/*.yaml')
}
...
_gh_merge_layer() {
  ...
  merged=$("$yq_bin" eval-all 'select(fileIndex == 0) * select(fileIndex == 1)' "$dst" "$src") || {
    _warn "    warning: GitHub CLI merge failed for $src — skipping"
    return 0
  }

  _merge_hook_write_text_if_changed "$dst" "$merged"
}
...
_gh_token_seed_failure_stamp_path() {
  _dot_xdg_path state "dot/gh-token-seed-failed.stamp"
}
